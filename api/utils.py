...
def load_model():
    ...
    weights_lst = ['BiRefNet-massive-epoch_240.pth','BiRefNet-DIS-bb_swin_v1_tiny-epoch_575.pth','BiRefNet-massive-bb_swin_v1_tiny-epoch_235.pth']
    weights_path = os.path.join(config.weights_root_dir, weights_lst[-3])

    model = BiRefNet(bb_pretrained=False)
    state_dict = torch.load(weights_path, map_location='cpu')
    state_dict = utils.check_state_dict(state_dict)
